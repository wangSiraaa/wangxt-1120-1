...
CREATE TABLE borrow_application (
    ...
    machine_code VARCHAR(64) NOT NULL COMMENT '机台编号',
    clean_level VARCHAR(16) NOT NULL COMMENT '申请洁净等级：CLASS_10 CLASS_100 CLASS_1000',
    purpose VARCHAR(512) COMMENT '借用用途',
    expect_return_date DATE NOT NULL COMMENT '预计归还日期',
    apply_status VARCHAR(32) NOT NULL DEFAULT 'PENDING' COMMENT '申请状态：PENDING-待审批 APPROVED-已批准 REJECTED-已驳回 CANCELLED-已取消 BORROWED-已借出 RETURNED-已归还 SUPERVISOR_REVIEW-主管复核中',
    supervisor_review_flag TINYINT NOT NULL DEFAULT 0 COMMENT '是否需主管复核：0-否 1-是',
    supervisor_review_reason VARCHAR(512) COMMENT '主管复核原因',
...
CREATE TABLE borrow_order (
    ...
    order_status VARCHAR(32) NOT NULL DEFAULT 'CREATED' COMMENT '单据状态：CREATED-已创建 OUT_STOCK-已出库 IN_STOCK-已入库 CLOSED-已关闭 SUPERVISOR_REVIEW-主管复核中 REJECTED-已驳回',
    ...
    appearance_check_result VARCHAR(32) COMMENT '外观检查结果：PASS-合格 FAIL-不合格 SCRATCH-划伤',
    ...
    machine_code VARCHAR(64) COMMENT '机台编号',
    clean_level VARCHAR(16) COMMENT '借用洁净等级',
    supervisor_review_status VARCHAR(32) NOT NULL DEFAULT 'NONE' COMMENT '主管复核状态：NONE-无需 PENDING-待复核 APPROVED-已通过 REJECTED-已驳回',
    supervisor_review_user_id BIGINT COMMENT '主管复核人ID',
    supervisor_review_time DATETIME COMMENT '主管复核时间',
    supervisor_review_remark VARCHAR(512) COMMENT '主管复核意见',
    batch_freeze_flag TINYINT NOT NULL DEFAULT 0 COMMENT '批次冻结标志：0-未冻结 1-已冻结',
...
-- 8. 库位移动记录表
DROP TABLE IF EXISTS mask_location_move;
CREATE TABLE mask_location_move (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
    move_no VARCHAR(64) NOT NULL UNIQUE COMMENT '移动记录编号',
    mask_id BIGINT NOT NULL COMMENT '光罩ID',
    mask_code VARCHAR(64) COMMENT '光罩编码',
    mask_name VARCHAR(128) COMMENT '光罩名称',
    borrow_order_id BIGINT COMMENT '关联借用单ID',
    ...
    from_location_code VARCHAR(64) COMMENT '源库位编码',
    from_clean_level VARCHAR(16) COMMENT '源库位洁净等级',
    to_location_id BIGINT COMMENT '目标库位ID',
    to_location_code VARCHAR(64) COMMENT '目标库位编码',
    to_clean_level VARCHAR(16) COMMENT '目标库位洁净等级',
    move_type VARCHAR(32) NOT NULL COMMENT '移动类型：OUT_STOCK-出库 IN_STOCK-入库',
    move_reason VARCHAR(512) COMMENT '移动原因',
    clean_level_mismatch VARCHAR(4) NOT NULL DEFAULT 'NO' COMMENT '洁净等级是否不匹配：YES-是 NO-否',
    ...
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    deleted TINYINT NOT NULL DEFAULT 0 COMMENT '逻辑删除：0-未删除 1-已删除',
    INDEX idx_move_no (move_no),
    INDEX idx_mask_id (mask_id),
    INDEX idx_borrow_order_id (borrow_order_id),
    INDEX idx_move_type (move_type),
    INDEX idx_clean_level_mismatch (clean_level_mismatch),
    INDEX idx_move_time (move_time)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='库位移动记录表';

-- 9. 外观检查照片表
DROP TABLE IF EXISTS appearance_check_photo;
CREATE TABLE appearance_check_photo (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
    borrow_order_id BIGINT NOT NULL COMMENT '借用单ID',
    order_no VARCHAR(64) COMMENT '借用单号',
    return_record_id BIGINT COMMENT '归还记录ID',
    mask_id BIGINT NOT NULL COMMENT '光罩ID',
    mask_code VARCHAR(64) COMMENT '光罩编码',
    photo_path VARCHAR(512) NOT NULL COMMENT '照片存储路径',
    photo_name VARCHAR(256) COMMENT '照片文件名',
    photo_size BIGINT COMMENT '照片文件大小（字节）',
    check_result VARCHAR(32) COMMENT '检查结果：PASS-合格 FAIL-不合格 SCRATCH-划伤',
    check_type VARCHAR(32) NOT NULL DEFAULT 'RETURN' COMMENT '检查类型：RETURN-归还检查 OUT-出库检查',
    check_user_id BIGINT COMMENT '检查人ID',
    check_user_name VARCHAR(128) COMMENT '检查人姓名',
    ...
    deleted TINYINT NOT NULL DEFAULT 0 COMMENT '逻辑删除：0-未删除 1-已删除',
    INDEX idx_borrow_order_id (borrow_order_id),
    INDEX idx_return_record_id (return_record_id),
    INDEX idx_mask_id (mask_id),
    INDEX idx_check_type (check_type),
    INDEX idx_check_time (check_time)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='外观检查照片表';

-- 10. 异常处理单表
DROP TABLE IF EXISTS anomaly_handle_order;
CREATE TABLE anomaly_handle_order (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
    anomaly_no VARCHAR(64) NOT NULL UNIQUE COMMENT '异常单号',
    borrow_order_id BIGINT COMMENT '关联借用单ID',
    order_no VARCHAR(64) COMMENT '借用单号',
    mask_id BIGINT NOT NULL COMMENT '光罩ID',
    mask_code VARCHAR(64) COMMENT '光罩编码',
    mask_name VARCHAR(128) COMMENT '光罩名称',
    machine_batch VARCHAR(128) COMMENT '机台批次',
    anomaly_type VARCHAR(32) NOT NULL COMMENT '异常类型：SCRATCH_DAMAGE-划伤 CLEAN_LEVEL_MISMATCH-洁净区不匹配 OTHER-其他',
    anomaly_desc VARCHAR(1024) COMMENT '异常描述',
    report_user_id BIGINT COMMENT '上报人ID',
    report_user_name VARCHAR(128) COMMENT '上报人姓名',
    report_time DATETIME COMMENT '上报时间',
    handle_status VARCHAR(32) NOT NULL DEFAULT 'PENDING' COMMENT '处理状态：PENDING-待处理 HANDLED-已处理 UNFROZEN-已解冻',
    ...
    batch_freeze_flag TINYINT NOT NULL DEFAULT 0 COMMENT '批次冻结标志：0-未冻结 1-已冻结',
    batch_freeze_reason VARCHAR(512) COMMENT '批次冻结原因',
    batch_freeze_time DATETIME COMMENT '批次冻结时间',
    batch_unfreeze_time DATETIME COMMENT '批次解冻时间',
    batch_unfreeze_user_id BIGINT COMMENT '批次解冻人ID',
    remark VARCHAR(512) COMMENT '备注',
    create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    deleted TINYINT NOT NULL DEFAULT 0 COMMENT '逻辑删除：0-未删除 1-已删除',
    INDEX idx_anomaly_no (anomaly_no),
    INDEX idx_borrow_order_id (borrow_order_id),
    INDEX idx_mask_id (mask_id),
    INDEX idx_anomaly_type (anomaly_type),
    INDEX idx_handle_status (handle_status),
    INDEX idx_machine_batch (machine_batch),
    INDEX idx_batch_freeze_flag (batch_freeze_flag)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='异常处理单表';
